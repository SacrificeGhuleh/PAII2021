...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class T>
__host__ void checkHostMatrix(const T *m, const unsigned int pitchInBytes, const unsigned int rows, const unsigned int cols, const char *format = "%f ",
                              const char *message = "") {
#ifdef __CHECK_DATA_
  printf("\nHOST MEMORY: %s [%u %u]\n", message, rows, cols);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class T>
__host__ void checkDeviceMatrix(const T *m, const unsigned int pitchInBytes, const unsigned int rows, const unsigned int cols, const char *format = "%f ",
                                const char *message = "") {
#ifdef __CHECK_DATA_
  printf("\nDEVICE MEMORY: %s [%u %u]\n", message, rows, cols);
  T *ptr;
...
template<class T>
__host__ void
checkDeviceArray(const cudaArray *m, const unsigned int pitchInBytes, const unsigned int rows, const unsigned int cols, const char *format = "%f ",
                 const char *message = "") {
#ifdef __CHECK_DATA_
  printf("\nDEVICE MEMORY: %s [%u %u]\n", message, rows, cols);
  T *ptr;
...
template<class T>
__host__ void
verifyDeviceMatrix(const T *m, const unsigned int pitchInBytes, const unsigned int rows, const unsigned int cols, const T minValue, const T maxValue,
                   const char *message = "") {
#ifdef __CHECK_DATA_
  printf("\nDEVICE MEMORY: %s [%u %u]\n", message, rows, cols);
  T *ptr;
...
struct check_data {
  ...
  checkDeviceMatrix(const T *m, const unsigned int rows, const unsigned int cols, const bool isRowMatrix = true, const char *format = "%f ",
                    const char *message = "") {}
};

template<>
struct check_data<float4> {
  static __host__ void
  checkDeviceMatrix(const float4 *m, const unsigned int rows, const unsigned int cols, const bool isRowMatrix = true, const char *format = "%f %f %f %f ",
                    const char *message = "") {
#ifdef __CHECK_DATA_
...
template<>
struct check_data<uchar4> {
  static __host__ void checkDeviceMatrix(const uchar4 *m, const unsigned int rows, const unsigned int cols, const bool isRowMatrix = true,
                                         const char *format = "%hhu %hhu %hhu %hhu ", const char *message = "") {
#ifdef __CHECK_DATA_
    printf("\n------------------------- DEVICE MEMORY: %s [%u %u] %s\n", message, rows, cols, (isRowMatrix) ? "row matrix" : "column matrix ");
    uchar4 *tmp;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class T>
__host__ void checkDeviceMatrixCUBLAS(const T *m, const unsigned int rows, const unsigned int cols, const bool isRowMatrix = true, const char *format = "%f ",
                                      const char *message = "") {
#ifdef __CHECK_DATA_
  printf("\n------------------------- DEVICE MEMORY: %s [%u %u] %s\n", message, rows, cols, (isRowMatrix) ? "row matrix" : "column matrix ");
...
__forceinline__  __host__ bool checkDeviceProperties() {
  ...
  for (dev = 0; dev < deviceCount; ++dev) {
    ...
    printf("  \x1B[91m%-50s: %llu bytes\033[0m\n", "Total amount of shared memory per SM",
           static_cast<long long unsigned int>(deviceProp.sharedMemPerMultiprocessor));
    ...
    printf("  %-50s: %d, %d, %d\n", "Maximum sizes of each dimension of a block", deviceProp.maxThreadsDim[0], deviceProp.maxThreadsDim[1],
           deviceProp.maxThreadsDim[2]);
    printf("  %-50s: %d, %d, %d\n", "Maximum sizes of each dimension of a grid", deviceProp.maxGridSize[0], deviceProp.maxGridSize[1],
...
prepareKernelSettings(const cudaDeviceProp &deviceProp, const unsigned int initNoChunks, const unsigned int threadsPerBlock, const unsigned int dataLength,
                      KernelSetting &ks) {
...
__forceinline__  __host__ void CUDA_SAFE_CALL(cudaError_t call, int line) {
  switch (call) {
    ...
    default:printf("ERROR at line :%i.%d' ' %s\n", line, call, cudaGetErrorString(call));
      exit(-1);
      break;
  }
}
